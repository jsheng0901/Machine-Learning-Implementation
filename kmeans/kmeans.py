...
class KMeans:
    ...
    def closest_centroid(self, sample, centroids):
        ...
        for i, centroid in enumerate(centroids):        # loop through all centroids check which one is close to sample
    ...
    def calculate_centroids(self, clusters, X):
        ...
        for i, cluster in enumerate(clusters):
            centroid = np.mean(X[cluster], axis=0)      # get each clusters index and calculate each feature mean
            centroids[i] = centroid
        return centroids
    ...
    def get_cluster_labels(self, clusters, X):
        ...
        for cluster_i, cluster in enumerate(clusters):
            for sample_i in cluster:
                y_pred[sample_i] = cluster_i        # assign each sample inside which cluster with this cluster label
        return y_pred
